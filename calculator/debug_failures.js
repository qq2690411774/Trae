// 测试脚本 - 用于诊断 auto_test.html 中的失败测试
let currentInput = '0';
let previousInput = '';
let operator = null;
...
function inputNumber(num) {
    if (num === '(' || num === ')') {
        if (num === '(') {
            ...
            if (openCount > closeCount) {
                // 如果前面有 pending 操作，需要把完整表达式组合起来
                if (operator && previousInput) {
                    // previousInput 包含了 '(2', currentInput 包含了 '(25'
                    // 需要提取 currentInput 中括号内的实际内容
                    let innerContent = currentInput;
                    
                    // 找到 currentInput 中最后一个 ( 的位置
                    const lastOpenIndex = innerContent.lastIndexOf('(');
                    if (lastOpenIndex >= 0) {
                        innerContent = innerContent.substring(lastOpenIndex + 1);
                    }
                    
                    // 去掉 previousInput 中开头和末尾的括号
                    let cleanPrev = previousInput;
                    while (cleanPrev.startsWith('(')) {
                        cleanPrev = cleanPrev.substring(1);
                    }
                    while (cleanPrev.endsWith(')')) {
                        cleanPrev = cleanPrev.substring(0, cleanPrev.length - 1);
                    }
                    // 去掉末尾的操作符
                    if (operator && cleanPrev.length > 0) {
                    ...
                    // 组合成完整的括号表达式
                    currentInput = '(' + cleanPrev + operator + innerContent + ')';
                    previousInput = '';
                    operator = null;
                } else {
                    currentInput += ')';
    ...
    // 如果有未闭合括号，即使 shouldResetDisplay=true 也不重置，而是继续追加
...
function inputOperator(op) {
    ...
    // 如果有 pending 的括号表达式在 previousInput 中，不评估，等用户输入 )
    // 只有在计算时才评估
